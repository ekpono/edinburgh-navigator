          ...
          <div className="space-y-4">
              ...
              <div className="space-y-3">
                {[
                  ...
                  <div key={item.group} className="flex items-start gap-3 border border-slate-100 rounded-xl p-3">
                    <span className={`text-lg flex-shrink-0 ${item.eligible ? "text-emerald-700" : "text-slate-200"}`}>
                      {item.eligible ? "✓" : "✗"}
                    </span>
                    <div>
              ...
              <div className="space-y-3">
                {[
                  ...
                  <div key={s.step} className="flex gap-3">
                    <div className="size-6 rounded-full bg-slate-900 text-white flex items-center justify-center text-xs font-bold flex-shrink-0">{s.step}</div>
                    <div>
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              <a href="https://www.edinburgh.gov.uk/childcare" target="_blank" rel="noreferrer"
                ...
                <div className="text-xs text-slate-200">Official Edinburgh City Council portal →</div>
              </a>
        ...
        {tab === "finding" && (
          <div className="space-y-4">
                    ...
                    <div className="flex items-start justify-between gap-2">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <div className="text-xs text-slate-700 font-semibold">{t.ages}</div>
                        </div>
                      </div>
                      <span className="text-xs bg-emerald-50 text-emerald-700 px-2 py-1 rounded-full font-semibold flex-shrink-0">{t.cost}</span>
                    </div>
                    <p className="text-xs text-slate-700 mt-2 leading-relaxed">{t.desc}</p>
                  </div>
                ))}
              </div>
            </div>
              ...
              <div className="space-y-2">
                {[
                  ...
                  <a key={r.name} href={r.url} target="_blank" rel="noreferrer"
                    ...
                    <span className="text-slate-700 font-bold text-xs flex-shrink-0 ml-2">→</span>
                  </a>
                ))}
              </div>
            </div>
            ...
            <div className="border-l-2 border-amber-600 bg-amber-50 px-5 py-4 max-w-3xl">
              <strong className="text-xs text-amber-800">Tax-Free Childcare (UK-wide)</strong>
              ...
              <a href="https://www.childcarechoices.gov.uk/" target="_blank" rel="noreferrer"
                className="inline-block mt-2 text-xs font-bold text-amber-800 hover:underline">Apply for Tax-Free Childcare →</a>
            </div>
          </div>
        )}
        ...
        {tab === "benefits" && (
          <div className="space-y-4">
            ...
            <div className="bg-slate-900 text-white rounded-xl p-5">
              ...
              <p className="text-xs text-slate-200 mt-1 mb-3">Use the Social Security Scotland calculator or speak to Citizens Advice Edinburgh — many families miss out on thousands of pounds a year.</p>
              <div className="flex flex-wrap gap-2">
                <a href="https://www.socialsecurity.gov.scot/" target="_blank" rel="noreferrer"
                  className="text-xs font-bold bg-slate-900 text-white px-3 py-2 rounded-lg hover:bg-slate-900 transition-colors">
        ...
        {tab === "support" && (
          <div className="space-y-4">
              ...
              <div className="space-y-3">
                {[
                  ...
                  <div key={s.name} className="border border-slate-200 rounded-xl p-4">
                    ...
                    <p className="text-xs text-slate-700 mt-1.5 leading-relaxed">{s.desc}</p>
                    <div className="flex flex-wrap gap-3 mt-2">
                      <a href={`tel:${s.num}`} className="text-xs font-mono font-bold text-slate-700">📞 {s.num}</a>
                      <a href={s.link} target="_blank" rel="noreferrer" className="text-xs font-bold text-slate-700 hover:underline">Website →</a>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="border-l-2 border-emerald-700 bg-emerald-50 px-5 py-4 max-w-3xl">
              <strong className="text-sm text-emerald-900">Health visitor support</strong>
