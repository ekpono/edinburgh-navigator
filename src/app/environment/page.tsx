...
export default function EnvironmentPage() {
  ...
  return (
    <div className="min-h-full">
      <PageHeader section="The city" title="Clean Edinburgh" subtitle="Report, recycle, and keep Edinburgh green" />

            <PageIntro label="In brief">
        Edinburgh Council handles waste, recycling, and environmental issues across the city. Report problems and understand your rights.
      </PageIntro>
      <Band label="The council" ground="paper">
        ...
        {tab === "report" && (
          <div className="space-y-4">
            <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-5">
              ...
              <p className="text-xs text-emerald-900 mb-3">Edinburgh Council accepts reports for fly-tipping, potholes, broken streetlights, graffiti, abandoned vehicles, and more.</p>
              <a href="https://www.edinburgh.gov.uk/report" target="_blank" rel="noreferrer"
                className="inline-block bg-emerald-700 text-white text-xs font-bold px-4 py-2.5 rounded-xl hover:bg-emerald-900 transition-colors">
          ...
          <div className="space-y-4">
            <div className="bg-white rounded-xl border border-slate-200 p-5">
              ...
              <a href="https://www.edinburgh.gov.uk/bin-collection-days" target="_blank" rel="noreferrer"
                className="inline-block bg-slate-900 text-white text-xs font-bold px-4 py-2.5 rounded-xl hover:bg-slate-900 transition-colors">
                Check your bin day →
              </a>
            </div>
            {EDINBURGH_BIN_GUIDE.map((bin) => (
              <div key={bin.bin} className="bg-white rounded-xl border border-slate-200 p-5">
                <div className="flex items-center gap-2 mb-3">
                  <div className={`size-4 rounded-sm ${bin.bin.startsWith("Blue") ? "bg-slate-900" : bin.bin.startsWith("Green") ? "bg-emerald-700" : "bg-slate-500"}`} />
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <ul className="space-y-1">
                      {bin.contents.map((item) => <li key={item} className="text-xs text-slate-700">{item}</li>)}
                    </ul>
                  </div>
                  <div>
                    <div className="text-xs font-semibold text-red-600 mb-1">❌ Does NOT go in:</div>
                    <ul className="space-y-1">
                      {bin.not.map((item) => <li key={item} className="text-xs text-slate-700">{item}</li>)}
                    </ul>
                  </div>
                </div>
              </div>
            ...
            <div className="border-l-2 border-amber-600 bg-amber-50 px-5 py-4 max-w-3xl">
              <strong className="text-xs text-amber-800">Contamination fines</strong>
        ...
        {tab === "bulky" && (
          <div className="space-y-4">
            <div className="bg-white rounded-xl border border-slate-200 p-5">
              ...
              <p className="text-xs text-slate-700 mb-3 leading-relaxed">Edinburgh Council offers a bulky waste collection service for large items (furniture, white goods, mattresses). The charge depends on the number of items.</p>
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-5">
              ...
              {RECYCLING_CENTRES.map((centre) => (
                <div key={centre.name} className="border-b border-slate-100 last:border-0 py-3">
                  ...
                  <div className="text-xs text-emerald-700">🕐 {centre.hours}</div>
                </div>
              ))}
            </div>
            {[
        ...
        {tab === "green" && (
          <div className="space-y-3">
            <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-5">
              ...
              <p className="text-xs text-emerald-900 leading-relaxed">City of Edinburgh Council has committed to becoming net zero by 2030. The city has an extensive cycling infrastructure, 112 parks, and the Water of Leith corridor.</p>
            </div>
            {[
              ...
              <div key={park.name} className="bg-white rounded-xl border border-slate-200 p-4 flex items-start gap-3">
                ...
                <div>
                  ...
                  <div className="text-xs text-slate-700 mt-0.5 leading-relaxed">{park.desc}</div>
                  <div className="text-xs text-emerald-700 mt-1 font-medium">{park.address}</div>
                </div>
              </div>
            ))}
          </div>
